...
def get_manga_line(np_img: np.ndarray) -> np.ndarray:
    r"""
    Get the lineart of an numpy image.
    """
    global DEFAULT_EXTRACTOR
    if DEFAULT_EXTRACTOR is None:
        DEFAULT_EXTRACTOR = MangaLineExtration()
    line = DEFAULT_EXTRACTOR(np_img)
    return line

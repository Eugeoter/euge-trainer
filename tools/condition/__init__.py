import cv2
import numpy as np
from PIL import Image
from typing import Literal, Union, Dict, Any
from .utils import *

CNAUX_PROCESSORS = {}
CNAUX_CONTROL_TYPES = [
    "canny", "depth_leres", "depth_leres++", "depth_midas", "depth_zoe", "lineart_anime",
    "lineart_coarse", "lineart_realistic", "mediapipe_face", "mlsd", "normal_bae", "normal_midas",
    "openpose", "openpose_face", "openpose_faceonly", "openpose_full", "openpose_hand",
    "scribble_hed", "scribble_pidinet", "shuffle", "softedge_hed", "softedge_hedsafe",
    "softedge_pidinet", "softedge_pidsafe", "dwpose"
]
ADDITIONAL_CONTROL_TYPES = [
    "random_canny", "animalpose", "densepose", "teed", "mteed", "tile", "mobile_sam", "random_blur"
]


def get_controlnet_aux_condition(
    image: Union[Dict[str, Any], Image.Image],
    control_type: Literal[
        "canny", "depth_leres", "depth_leres++", "depth_midas", "depth_zoe", "lineart_anime",
        "lineart_coarse", "lineart_realistic", "mediapipe_face", "mlsd", "normal_bae", "normal_midas",
        "openpose", "openpose_face", "openpose_faceonly", "openpose_full", "openpose_hand",
        "scribble_hed", "scribble_pidinet", "shuffle", "softedge_hed", "softedge_hedsafe",
        "softedge_pidinet", "softedge_pidsafe", "dwpose",
        "random_canny", "animalpose", "densepose", "teed", "mteed", "tile", "mobile_sam", "random_blur"
    ],
    **kwargs
) -> np.ndarray:
    r"""
    Get the condition of an image using controlnet_aux library.
    """
    from controlnet_aux.processor import Processor
    # Options are:
    # ["canny", "depth_leres", "depth_leres++", "depth_midas", "depth_zoe", "lineart_anime",
    #  "lineart_coarse", "lineart_realistic", "mediapipe_face", "mlsd", "normal_bae", "normal_midas",
    #  "openpose", "openpose_face", "openpose_faceonly", "openpose_full", "openpose_hand",
    #  "scribble_hed, "scribble_pidinet", "shuffle", "softedge_hed", "softedge_hedsafe",
    #  "softedge_pidinet", "softedge_pidsafe", "dwpose"]
    if control_type in ADDITIONAL_CONTROL_TYPES:
        if control_type == "random_canny":
            return get_random_canny_condition(image)
        elif control_type == "animalpose":
            return get_animalpose_condition(image)
        elif control_type == "densepose":
            return get_densepose_condition(image)
        elif control_type == "teed":
            return get_teed_condition(image)
        elif control_type == "mteed":
            return get_mteed_condition(image)
        elif control_type == "tile":
            return get_random_tile_condition(image, **kwargs)
        elif control_type == "mobile_sam":
            return get_mobile_sam_condition(image, **kwargs)
        elif control_type == 'random_blur':
            return get_random_blur_condition(image, **kwargs)
    elif control_type in CNAUX_CONTROL_TYPES:
        if control_type not in CNAUX_PROCESSORS:
            CNAUX_PROCESSORS[control_type] = Processor(control_type, params=kwargs)
        processor = CNAUX_PROCESSORS[control_type]
        if isinstance(image, dict):
            img_path = image['image_path']
            image = Image.open(img_path)
        condition = processor(image, to_pil=True)
        return np.array(condition)
    else:
        raise ValueError(f"Invalid control type: {control_type}. Expected one of {', '.join(CNAUX_CONTROL_TYPES + ADDITIONAL_CONTROL_TYPES)}")


def get_mlsd_condition(image: Union[Dict[str, Any], Image.Image], thr_v=0.5, thr_d=0.5) -> np.ndarray:
    from .mlsd import get_mlsd
    if isinstance(image, dict):
        img_path = image['image_path']
        img = cv2.imread(img_path)
    else:
        img = np.array(image)
    mlsd_condition = get_mlsd(img, thr_v, thr_d)
    return mlsd_condition


def get_random_canny_condition(image: Union[Dict[str, Any], Image.Image], low_threshold=None, high_threshold=None) -> np.ndarray:
    from .canny import get_canny
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    canny_condition = get_canny(image, low_threshold, high_threshold)
    return canny_condition


def get_hed_condition(image: Union[Dict[str, Any], Image.Image]) -> np.ndarray:
    from .hed import get_hed
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    hed_condition = get_hed(image)
    return hed_condition


def get_midas_depth_condition(image: Union[Dict[str, Any], Image.Image], a=np.pi * 2.0, bg_th=0.1) -> np.ndarray:
    from .midas import get_midas_depth
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    midas_condition = get_midas_depth(image, a, bg_th)
    if midas_condition.shape[:2] != image.shape[:2]:
        midas_condition = cv2.resize(midas_condition, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_LANCZOS4)
    return midas_condition


def get_midas_normal_condition(image: Union[Dict[str, Any], Image.Image], a=np.pi * 2.0, bg_th=0.1) -> np.ndarray:
    from .midas import get_midas_normal
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    midas_condition = get_midas_normal(image, a, bg_th)
    if midas_condition.shape[:2] != image.shape[:2]:
        midas_condition = cv2.resize(midas_condition, (image.shape[1], image.shape[0]), interpolation=cv2.INTER_LANCZOS4)
    return midas_condition


def get_openpose_condition(image: Union[Dict[str, Any], Image.Image], include_body=True, include_hand=False, include_face=False) -> np.ndarray:
    from .openpose import get_openpose
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    openpose_condition = get_openpose(image, include_body=include_body, include_hand=include_hand, include_face=include_face)
    return openpose_condition


def get_dwpose_condition(image: Union[Dict[str, Any], Image.Image], include_body=True, include_hand=False, include_face=False) -> np.ndarray:
    from .openpose import get_dwpose
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    openpose_condition = get_dwpose(image, include_body=include_body, include_hand=include_hand, include_face=include_face)
    return openpose_condition


def get_animalpose_condition(image: Union[Dict[str, Any], Image.Image], include_body=True, include_hand=False, include_face=False) -> np.ndarray:
    from .openpose import get_animalpose
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    openpose_condition = get_animalpose(image, include_body=include_body, include_hand=include_hand, include_face=include_face)
    return openpose_condition


def get_densepose_condition(image: Union[Dict[str, Any], Image.Image], cmap='viridis') -> np.ndarray:
    from .densepose import get_densepose
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    densepose_condition = get_densepose(image, cmap=cmap)
    return densepose_condition


def get_sketch_condition(image: Union[Dict[str, Any], Image.Image]) -> np.ndarray:
    from .sketch import get_sketch
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    sketch_condition = get_sketch(image)
    return sketch_condition


def get_lineart_condition(image: Union[Dict[str, Any], Image.Image], model_name='sk_model.pth', white_bg=False) -> np.ndarray:
    from .lineart import get_lineart
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    lineart_condition = get_lineart(image, model_name=model_name, white_bg=white_bg)
    return lineart_condition


def get_manga_line_condition(image: Union[Dict[str, Any], Image.Image]) -> np.ndarray:
    from .manga_line import get_manga_line
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    manga_line_condition = get_manga_line(image)
    return manga_line_condition


def get_pidinet_condition(image: Union[Dict[str, Any], Image.Image], is_safe=False, apply_fliter=False) -> np.ndarray:
    from .pidinet import get_pidinet
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    scribble_pidinet_condition = get_pidinet(image, is_safe=is_safe, apply_fliter=apply_fliter)
    return scribble_pidinet_condition


def get_teed_condition(image: Union[Dict[str, Any], Image.Image]) -> np.ndarray:
    from .teed import get_teed
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    teeds_condition = get_teed(image, mteed=False)
    return teeds_condition


def get_mteed_condition(image: Union[Dict[str, Any], Image.Image]) -> np.ndarray:
    from .teed import get_teed
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    teeds_condition = get_teed(image, mteed=True)
    return teeds_condition


def get_mobile_sam_condition(image: Union[Dict[str, Any], Image.Image], detect_resolution=512, image_resolution=512) -> np.ndarray:
    from .mobile_sam import get_mobile_sam
    if isinstance(image, dict):
        img_path = image['image_path']
        image = cv2.imread(img_path)
    else:
        image = np.array(image)
    sam_condition = get_mobile_sam(image, detect_resolution=detect_resolution, image_resolution=image_resolution)
    return sam_condition


def get_random_tile_condition(image: Union[Dict[str, Any], Image.Image], k=None, interpolation_downscale=None, interpolation_upscale=None) -> np.ndarray:
    from .tile import get_tile
    if isinstance(image, dict):
        img_path = image['image_path']
        image = Image.open(img_path)
    tile_condition = get_tile(image, k, interpolation_downscale, interpolation_upscale)
    return np.array(tile_condition)


def get_random_blur_condition(image: Union[Dict[str, Any], Image.Image], radius=None) -> np.ndarray:
    from .blur import get_blur
    if isinstance(image, dict):
        img_path = image['image_path']
        image = Image.open(img_path)
    blur_condition = get_blur(image, radius)
    return np.array(blur_condition)


def get_random_inpainting_mask_condition(
    image: Union[Dict[str, Any], Image.Image],
    box_proba=1/2,
    bbox_min_size=30,
    bbox_max_size=400,
    bbox_min_times=0,
    bbox_max_times=3,
    irregular_proba=1/2,
    irregular_max_angle=6,
    irregular_max_len=240,
    irregular_max_width=80,
    irregular_min_times=0,
    irregular_max_times=3,
    superres_proba=0,
    segm_proba=0,
    variants_n=1,
) -> np.ndarray:
    from .inpaint import get_random_inpaint_mask
    if isinstance(image, dict):
        img_path = image['image_path']
        image = Image.open(img_path)
    inpaint_condition = get_random_inpaint_mask(
        image,
        box_proba=box_proba,
        bbox_min_size=bbox_min_size,
        bbox_max_size=bbox_max_size,
        bbox_min_times=bbox_min_times,
        bbox_max_times=bbox_max_times,
        irregular_proba=irregular_proba,
        irregular_max_angle=irregular_max_angle,
        irregular_max_len=irregular_max_len,
        irregular_max_width=irregular_max_width,
        irregular_min_times=irregular_min_times,
        irregular_max_times=irregular_max_times,
        superres_proba=superres_proba,
        segm_proba=segm_proba,
        variants_n=variants_n,
    )
    return np.array(inpaint_condition)


def get_random_inpainting_masked_image_condition(
    image: Union[Dict[str, Any], Image.Image],
    box_proba=1/2,
    bbox_min_size=30,
    bbox_max_size=400,
    bbox_min_times=0,
    bbox_max_times=3,
    irregular_proba=1/2,
    irregular_max_angle=6,
    irregular_max_len=240,
    irregular_max_width=80,
    irregular_min_times=0,
    irregular_max_times=3,
    superres_proba=0,
    segm_proba=0,
    variants_n=1,
) -> np.ndarray:
    from .inpaint import get_random_inpaint_mask
    if isinstance(image, dict):
        img_path = image['image_path']
        image = Image.open(img_path)
    mask = get_random_inpaint_mask(
        image,
        box_proba=box_proba,
        bbox_min_size=bbox_min_size,
        bbox_max_size=bbox_max_size,
        bbox_min_times=bbox_min_times,
        bbox_max_times=bbox_max_times,
        irregular_proba=irregular_proba,
        irregular_max_angle=irregular_max_angle,
        irregular_max_len=irregular_max_len,
        irregular_max_width=irregular_max_width,
        irregular_min_times=irregular_min_times,
        irregular_max_times=irregular_max_times,
        superres_proba=superres_proba,
        segm_proba=segm_proba,
        variants_n=variants_n,
    )
    mask = mask.resize(image.size, Image.BILINEAR)
    mask = Image.eval(mask, lambda a: 255 - a)
    masked_image = Image.composite(image, Image.new('RGB', image.size, (0, 0, 0)), mask)
    return np.array(masked_image)


def get_random_outpainting_mask_condition(
    image: Union[Dict[str, Any], Image.Image],
    min_margin_ratio: float = 0.1,
    max_margin_ratio: float = 0.4,
) -> np.ndarray:
    from .outpaint import get_random_outpainting_mask
    if isinstance(image, dict):
        img_path = image['image_path']
        image = Image.open(img_path)
    mask = get_random_outpainting_mask(image, min_margin_ratio, max_margin_ratio)
    return np.array(mask)


def get_random_outpainting_masked_image_condition(
    image: Union[Dict[str, Any], Image.Image],
    min_margin_ratio: float = 0.1,
    max_margin_ratio: float = 0.4,
) -> np.ndarray:
    from .outpaint import get_random_outpainting_mask
    if isinstance(image, dict):
        img_path = image['image_path']
        image = Image.open(img_path)
    np_img = np.array(image)

    mask = get_random_outpainting_mask(image, min_margin_ratio, max_margin_ratio)
    mask = Image.fromarray(mask, mode='L').resize(image.size, Image.BILINEAR)
    mask_np = np.array(mask)

    masked_image = np_img.copy()
    masked_image[mask_np == 0] = 0

    return masked_image

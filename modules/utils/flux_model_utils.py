...
from .sd15_model_utils import MemoryEfficientSafeOpen
...
def load_flow_model(
    name: str, ckpt_path: str, dtype: torch.dtype, device: Union[str, torch.device], disable_mmap: bool = False, nnet_class=flux_models.Flux,
) -> flux_models.Flux:
    logger.info(f"Building Flux model {name}")
    with torch.device("meta"):
        model = nnet_class(flux_models.configs[name].params).to(dtype)

...
def get_prompts_with_weights(pipe: StableDiffusionPipeline, prompt: List[str], max_length: int):
    ...
    if truncated:
        logger.print(logging.magenta("Prompt was truncated. Try to shorten the prompt or increase max_embeddings_multiples"))
...
def get_hidden_states(text_encoder, input_ids, is_sdxl_text_encoder2: bool, eos_token_id, device):
    if not is_sdxl_text_encoder2:
        ...
        pool = sdxl_train_utils.pool_workaround(text_encoder, enc_out["last_hidden_state"], input_ids, eos_token_id)
...
class StableDiffusionXLControlNeXtPipeline:
    ...
    def decode_latents(self, latents):
        with torch.no_grad():
            latents = 1 / sdxl_model_utils.VAE_SCALE_FACTOR * latents
    ...
    def prepare_latents(self, image, timestep, batch_size, height, width, dtype, device, generator, latents=None):
        if image is None:
            ...
            init_latents = sdxl_model_utils.VAE_SCALE_FACTOR * init_latents
    ...
    def __call__(
        ...
        if controlnet_image is not None:
            controlnet_image = prepare_controlnet_image(
                controlnet_image, width, height, batch_size, num_images_per_prompt, self.device, self.controlnext.dtype, do_classifier_free_guidance, False
            )
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if self.controlnext is not None and controlnet_image is not None:
                ...
                # if do_classifier_free_guidance:
                #     N = controlnet_output['out'].shape[0]
                #     controlnet_output['scale'] = torch.tensor(controlnet_output['scale']).to(latent_model_input).repeat(N)[:, None, None, None]
                #     controlnet_output['scale'][:N // 2] *= 0
                #     # controlnet_output['scale'] *= 1
                # else:
                #     controlnet_output['scale'] = controlnet_output['scale']  # no change

                controlnet_output['out'] = controlnet_output['out'].to(dtype)
                controlnet_output['scale'] *= controlnet_scale
                unet_additional_args["controls"] = controlnet_output

            # predict the noise residual
            noise_pred = self.unet(latent_model_input, t, text_embedding, vector_embedding, **unet_additional_args)

...
class StableDiffusionXLControlNeXtPipeline(
    ...
    def __call__(
        ...
        **kwargs,
        ...
        print(f"add_time_ids: {add_time_ids.shape}, add_text_embeds: {add_text_embeds.shape}")
        ...
        if not output_type == "latent":
            ...
            controlnet_image = self.vae.decode(latents, return_dict=False)[0]

            # cast back to fp16 if needed
            if needs_upcasting:
                self.vae.to(dtype=torch.float16)
        else:
            controlnet_image = latents

        if not output_type == "latent":
            # apply watermark if available
            if self.watermark is not None:
                controlnet_image = self.watermark.apply_watermark(controlnet_image)

            controlnet_image = self.image_processor.postprocess(controlnet_image, output_type=output_type)
        ...
        return StableDiffusionXLPipelineOutput(images=controlnet_image)

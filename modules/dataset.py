...
def search_cache_file(image_dir, image_key):
    image_file = os.path.join(image_dir, f"{image_key}.npz")
    if os.path.isfile(image_file):
        image_path = os.path.abspath(image_file)
    else:
        image_path = None
...
class Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.shuffle_caption = args.shuffle_caption
        ...
        artist_benchmark = sum(counter["artist"].values()) / len(counter["artist"]) if len(counter["artist"]) > 0 else None
    ...
    def get_input_ids(self, caption, tokenizer):
        input_ids = tokenizer(
            caption, padding="max_length", truncation=True, max_length=self.max_token_length, return_tensors="pt"
        ).input_ids

        if self.max_token_length > tokenizer.model_max_length:
            ...
            if tokenizer.pad_token_id == tokenizer.eos_token_id:
                ...
                for i in range(
                    1, self.max_token_length - tokenizer.model_max_length +
                ...
                for i in range(1, self.max_token_length - tokenizer.model_max_length + 2, tokenizer.model_max_length - 2):
                    ids_chunk = (
    ...
    def __getitem__(self, index):
        ...
        for image_info in batch:
            ...
            # logu.debug(f"Load image: {image_info.image_key}")

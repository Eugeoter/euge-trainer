...
class SDXLControlNeXtUNet2DConditionModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        is_controlnet = mid_block_additional_residual is not None
        ...
        for i, module in enumerate(self.input_blocks):
            if i == 1 and controls is not None:
                scale = controls['scale']
                controls = controls['out'].to(h)
                ...
                # print(f"scale: {scale}, controls.shape: {controls.shape}, sample.shape: {h.shape}")
                controls = nn.functional.adaptive_avg_pool2d(controls, h.shape[-2:])
                # h = h + controls * scale * 0.35
                h = h + controls * scale
        ...
        #     scale = controls['scale']
        #     controls = controls['out'].to(h)
        #     # print(f"controls.shape: {controls.shape}, h.shape: {h.shape}")
        #     mean_latents, std_latents = torch.mean(h, dim=(1, 2, 3), keepdim=True), torch.std(h, dim=(1, 2, 3), keepdim=True)
        #     mean_control, std_control = torch.mean(controls, dim=(1, 2, 3), keepdim=True), torch.std(controls, dim=(1, 2, 3), keepdim=True)
        #     controls = (controls - mean_control) * (std_latents / (std_control + 1e-12)) + mean_latents
        #     controls = nn.functional.adaptive_avg_pool2d(controls, h.shape[-2:])
...
class InferSdxlUNet2DConditionModel:
    def __init__(self, original_unet: SDXLControlNeXtUNet2DConditionModel, **kwargs):
...
if __name__ == "__main__":
    ...
    unet = SDXLControlNeXtUNet2DConditionModel()

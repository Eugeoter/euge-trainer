...
class StableDiffusionControlNeXtPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    r"""
    ...
    def __call__(
        ...
        grid_height = height // 8 // self.unet.config.patch_size
        grid_width = width // 8 // self.unet.config.patch_size
        base_size = 512 // 8 // self.unet.config.patch_size
        grid_crops_coords = get_resize_crop_region_for_grid((grid_height, grid_width), base_size)
        cos_cis_img, sin_cis_img = get_2d_rotary_pos_embed(
            self.unet.hidden_size // self.unet.num_heads, grid_crops_coords, (grid_height, grid_width)
        )
        ...
        prompt_embeds_t5 = prompt_embeds_t5.to(device=device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # print(f"alpha_prod_t: {alpha_prod_t.shape}, t: {t}, latents: {latents.shape}, noise_pred: {noise_pred.shape}")

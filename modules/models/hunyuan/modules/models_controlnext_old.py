...
class HunYuanControlNeXtDiT(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    ...
    def forward(self,
        ...
        # print(f"text_states.shape: {text_states.shape}, text_states_t5.shape: {text_states_t5.shape}")
        # print(f"text_states_mask.shape: {text_states_mask.shape}, text_states_t5_mask.shape: {text_states_t5_mask.shape}")
        # print(f"b: {b_t5}, l: {l_t5}, c: {c_t5}")
        text_states = torch.cat([text_states, text_states_t5.view(b_t5, l_t5, -1)], dim=1)  # 2,205，1024
        clip_t5_mask = torch.cat([text_states_mask, text_states_t5_mask], dim=-1)
        # print(f"text_states_after.shape: {text_states.shape}, clip_t5_mask_after.shape: {clip_t5_mask.shape}")
        ...
        for layer, block in enumerate(self.blocks):
            if layer > self.depth // 2:
                if controls is not None:
                    skip = skips.pop()
                    # print(f"[debug] skip.shape: {skip.shape}, controls['out'].shape: {controls['out'].shape}")
                    scale = controls['scale']
                    signal = controls['out'].to(skip)
                    mean_latents, std_latents = torch.mean(skip, dim=(1, 2), keepdim=True), torch.std(skip, dim=(1, 2), keepdim=True)
...
def DiT_g_2(args, **kwargs):
    return HunYuanControlNeXtDiT(args, depth=40, hidden_size=1408, patch_size=2, num_heads=16, mlp_ratio=4.3637, **kwargs)


def DiT_XL_2(args, **kwargs):
    return HunYuanControlNeXtDiT(args, depth=28, hidden_size=1152, patch_size=2, num_heads=16, **kwargs)

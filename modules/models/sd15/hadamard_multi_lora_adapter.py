...
class HadamardMultiLoRAWrapper(nn.Module):
    def __init__(
        ...
        self.module_ratio = nn.Parameter(torch.tensor(init_module_ratio))
        self.lora_ratios = nn.ParameterList([nn.Parameter(torch.tensor(init_lora_ratios[i])) for i in range(self.num_loras)])
    ...
    def to(self, *args, **kwargs):
        ...
        self.module_ratio.data = self.module_ratio.data.to(*args, **kwargs)
        for param in self.lora_ratios:
    ...
    def merged_weight(self):
        merged_weight = self.module.weight * self.module_ratio
        for i in range(self.num_loras):
            up_weight, down_weight, alpha = self.up_weights[i], self.down_weights[i], self.alphas[i]
            if up_weight is None or down_weight is None or alpha is None:
                continue

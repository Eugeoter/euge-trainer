...
class CacheImageEmbeddingDataset(Dataset):
    r"""
    Helper class to cache image embeddings to disk.
    """
...
class WaifuScorerDataset(ScoreDataset):
    flip_aug: bool = True
    ...
    cache_emb_max_dataloader_n_workers: int = 0
    keep_cached_emb_in_memory: bool = True
    ...
    def get_basic_sample(self, batch, samples):
        ...
        for img_key in batch:
            ...
            if (cache := self.get_cache(img_md, update=self.keep_cached_emb_in_memory)):
                if not is_flipped:
                    img_emb = cache[self.cache_column]
                else:
                    img_emb = cache[self.cache_flipped_column]
                image = None
            else:
                img_emb = None
            ...
            if self.cache_emb:
                assert img_emb is not None, f"Image embeddings must be cached for `{img_key}`, but got {cache}"

            if len(img_emb.shape) == 1:
                img_emb = img_emb.unsqueeze(0)  # add batch dimension

            score = self.get_score(img_md)

            sample['image_keys'].append(img_key)
            sample['images'].append(image)
            sample['image_embeddings'].append(img_emb)
            sample['scores'].append(score)
            sample['is_flipped'].append(is_flipped)
        sample['image_embeddings'] = torch.cat(sample['image_embeddings'], dim=0) if sample['image_embeddings'][0] is not None else None
        sample['scores'] = torch.tensor(sample['scores'], dtype=torch.float32).unsqueeze(-1)
        return sample

    def get_data_existence(self, img_md):
        if os.path.exists(self.get_cache_path(img_md)):
            return True
        return super().get_data_existence(img_md)

    def _print_cache_img_emb_message(self):
        self.logger.print(logging.green(f"==================== START CACHING ===================="))
        self.logger.print(f"cache_img_emb: {self.cache_emb} | cache_to_disk: {self.cache_to_disk} | cache_only: {self.cache_only}")
        self.logger.print(f"async_cache: {self.async_cache} | check_cache_validity: {self.check_cache_validity}")
        self.logger.print(f"cache_img_emb_max_dataloader_n_workers: {self.cache_emb_max_dataloader_n_workers} | keep_cached_img_emb_in_memory: {self.keep_cached_emb_in_memory}")
        self.logger.print(f"cache_batch_size: {self.cache_batch_size}")

    def cache_all_image_embeddings(self, clip_model, clip_preprocessor, empty_cache=False):
        batches = self.make_cache_batches()
        if not batches:
            self.logger.info(logging.green(f"all image embeddings are cached"))
            return
        ...
        if batches:
            ...
            dataloader = DataLoader(
                dataset,
                batch_size=1,
                num_workers=self.cache_emb_max_dataloader_n_workers,
                pin_memory=True,
    ...
    def cache_sample_image_embeddings(self, sample, clip_model, clip_preprocessor, empty_cache=False):
        ...
        with torch.no_grad():
            batch_img_embs = ws_train_utils.encode_images(
                ...
                max_workers=self.cache_emb_max_dataloader_n_workers
            ).cpu()
            if self.flip_aug:
        ...
        for img_key in img_keys:
            ...
            if self.check_cache_validity:
                if not self.is_cache_supported(img_md):
                    continue
                cache = self.get_cache(img_md, update=self.keep_cached_emb_in_memory)
